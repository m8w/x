...
void main() {
    ...
    // Warp (psychedelic swirl). Enforce a small minimum so there's always
    // some organic distortion even when the preset sets warp=0.
    float warpAmt = max(u_warp, 0.5);
    float t = u_time * warpAmt * 0.5;
    float warpX = sin(t * 1.11 + uvC.y * 3.0) * warpAmt * 0.03;
    float warpY = cos(t * 0.93 + uvC.x * 2.5) * warpAmt * 0.03;
    ...
    // Decay only — gamma is applied once in the composite pass.
    // Applying gamma here too would compound it every feedback frame (pow(x, gamma^N)).
    color.rgb *= u_decay;
