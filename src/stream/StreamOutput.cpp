...
// ── encoder ───────────────────────────────────────────────────────────────────
//
// Apple Silicon M2 has a dedicated Media Engine that handles H.264 encode
// entirely off the CPU. h264_videotoolbox exposes it through FFmpeg with
// zero additional GPL dependencies.
//
// Pixel format: NV12 — Apple's native chroma format; avoids an extra
// YUV420P→NV12 conversion inside VideoToolbox on every frame.
//
// Key options for low-latency streaming:
//   realtime         = 1   — signals VideoToolbox to minimise latency
//   constant_bit_rate= 1   — CBR keeps RTMP buffer stable
//   allow_sw         = 0   — never fall back to software (we want Media Engine)
//   profile          = high — best compression per bit
...
bool StreamOutput::tryOpenEncoder(const char* /*unused*/, bool /*unused*/,
    ...
    if (!codec) {
        fprintf(stderr, "StreamOutput: h264_videotoolbox not found — "
                "is this a macOS build with VideoToolbox-enabled FFmpeg?\n");
        return false;
    }
    ...
    m_codecCtx->pix_fmt      = AV_PIX_FMT_NV12;   // native to Apple Silicon
    m_codecCtx->flags       |= AV_CODEC_FLAG_GLOBAL_HEADER;

    // Apple Silicon M2 Media Engine — optimised for live streaming
    av_opt_set_int(m_codecCtx->priv_data, "realtime",          1, 0);
    av_opt_set_int(m_codecCtx->priv_data, "constant_bit_rate", 1, 0);
    av_opt_set_int(m_codecCtx->priv_data, "allow_sw",          0, 0);
    av_opt_set    (m_codecCtx->priv_data, "profile",      "high", 0);

    if (avcodec_open2(m_codecCtx, codec, nullptr) < 0) {
        fprintf(stderr, "StreamOutput: avcodec_open2 failed for h264_videotoolbox\n");
        avcodec_free_context(&m_codecCtx);
        return false;
    }

    fprintf(stderr, "StreamOutput: encoder = h264_videotoolbox (Apple Media Engine)\n");
    return true;
}
...
bool StreamOutput::start(int width, int height, int bitrate_kbps_, int fps_) {
    ...
    // NV12 software frame — VideoToolbox reads this directly on Apple Silicon
