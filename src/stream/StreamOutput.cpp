...
// ── encoder ───────────────────────────────────────────────────────────────────
//
// AMD AMF (Advanced Media Framework) — AMD's native HW encoder on Windows.
// Works on all AMD GPUs with Video Coding Engine (VCE), from GCN onward.
// No GPL dependency. Requires the AMD driver to be installed; FFmpeg wraps
// the AMF SDK automatically when the driver DLLs are present.
//
// Streaming-optimised options:
//   usage = ultralowlatency   minimise encode latency for live RTMP
//   rc    = cbr               constant bitrate keeps the RTMP buffer stable
...
bool StreamOutput::tryOpenEncoder(const char* /*unused*/, bool /*unused*/,
    ...
    if (!codec) {
        fprintf(stderr, "StreamOutput: h264_amf not found — "
                "is this a Windows build with an AMD GPU and driver installed?\n");
        return false;
    }
    ...
    av_opt_set(m_codecCtx->priv_data, "usage", "ultralowlatency", 0);
    av_opt_set(m_codecCtx->priv_data, "rc",    "cbr",             0);

    if (avcodec_open2(m_codecCtx, codec, nullptr) < 0) {
        fprintf(stderr, "StreamOutput: avcodec_open2 failed for h264_amf\n");
        avcodec_free_context(&m_codecCtx);
        return false;
    }

    fprintf(stderr, "StreamOutput: encoder = h264_amf (AMD AMF / VCE)\n");
    return true;
}
