...
// ── encoder ───────────────────────────────────────────────────────────────────
//
// VA-API — Video Acceleration API, the Linux kernel's universal HW encode
// interface. Works with Intel (i915), AMD (amdgpu), and some ARM SoCs.
// No GPL dependency. Requires /dev/dri/renderD128 (or similar) to be present.
//
// Frame flow: RGB → swscale → NV12 (CPU) → av_hwframe_transfer_data → VAAPI
// The upload step is the only CPU touch per frame; the encode runs on the GPU.

bool StreamOutput::tryOpenEncoder(const char* /*unused*/, bool /*unused*/,
                                  int width, int height) {
    const AVCodec* codec = avcodec_find_encoder_by_name("h264_vaapi");
    if (!codec) {
        fprintf(stderr, "StreamOutput: h264_vaapi not found — "
                "is this a Linux build with VAAPI-enabled FFmpeg?\n");
        return false;
    }

    // Open VAAPI device (auto-selects /dev/dri/renderD128 by default)
    if (av_hwdevice_ctx_create(&m_hwDeviceCtx, AV_HWDEVICE_TYPE_VAAPI,
                               nullptr, nullptr, 0) < 0) {
        fprintf(stderr, "StreamOutput: failed to open VAAPI device — "
                "check /dev/dri/renderD128 exists and is accessible\n");
        return false;
    }

    // Hardware frames pool: VAAPI surface format, NV12 software backing
    ...
    if (av_hwframe_ctx_init(framesRef) < 0) {
        fprintf(stderr, "StreamOutput: av_hwframe_ctx_init failed\n");
    ...
    if (avcodec_open2(m_codecCtx, codec, nullptr) < 0) {
        fprintf(stderr, "StreamOutput: avcodec_open2 failed for h264_vaapi\n");
        av_buffer_unref(&m_hwDeviceCtx);
        avcodec_free_context(&m_codecCtx);
        return false;
    }

    fprintf(stderr, "StreamOutput: encoder = h264_vaapi (Intel/AMD VA-API)\n");
    return true;
}
...
bool StreamOutput::start(int width, int height, int bitrate_kbps_, int fps_) {
    ...
    // CPU-side NV12 frame — uploaded to VAAPI surface on each pushFrame
    ...
    // GPU-side VAAPI frame — what the encoder actually reads
...
void StreamOutput::pushFrame(const uint8_t* rgbData, int width, int height) {
    ...
    // Upload CPU NV12 → GPU VAAPI surface, then encode
